...
import TodoList from '../Chapter7/TodoList';
import TodoListItem from '../Chapter52/src/components/TodoListItem';
...
// <TodoProvider> {children=자식컴포넌트} </TodoProvider>
type PropsType = {
  children: JSX.Element | JSX.Element[];
};

// 상태와 상태 변경 함수를 가지는 컴포넌트
// 상태와 상태 변경 함수를 가지는 데이터 타입 형식으로 구성한 후 value 속성으로 전달
// <TodoContext.Provider value={value}>{children}</TodoContext>

export const TodoProvider = (props: PropsType) => {
  const [todoList, setTodoList] = useState<Array<TodoListItemType>>([
    {no: 1, todo: 'React 학습1', done: false},
    {no: 2, todo: 'React 학습2', done: true},
    {no: 3, todo: 'React 학습3', done: false},
